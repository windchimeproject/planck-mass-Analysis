...
def transform_temp(alphas, sensors_pos, lin_resp, adc_timestep_size=10 ** (-7), cylinder_cut_size=1):
    ...
    for alpha_index in tqdm(range(alphas.shape[0])):
        ...
        entry_vecs = np.array([[alpha_pair[0]], [alpha_pair[1]], [alpha_pair[2]]])
        exit_vecs = np.array([[alpha_pair[4]], [alpha_pair[5]], [alpha_pair[6]]])

        for sens_num, sensor_pos in enumerate(sensors_pos):
            P = sensor_pos
            track_vec = (exit_vecs - entry_vecs).T[0]
            entry_to_P = P - entry_vecs.T[0]
            exit_to_P = P - exit_vecs.T[0]
            track_dot_ex_P = np.dot(track_vec, exit_to_P)
            track_dot_en_P = np.dot(track_vec, entry_to_P)

            if (track_dot_ex_P > 0):
                min_dist = np.linalg.norm(P - exit_vecs.T[0])
            elif (track_dot_en_P < 0):
                min_dist = np.linalg.norm(P - entry_vecs.T[0])
            else:
            ...
            if min_dist < cylinder_cut_size:
                ...
                new_positions_array.append(np.array(P))
                sens_nums_array.append(sens_num)

        expected_signal_from_sensor['Alpha_num'].append(alpha_index)
        expected_signal_from_sensor['Signal'].append(np.array(signal_array))
        expected_signal_from_sensor['cut_sensor_positions'].append(np.array(new_positions_array))
        expected_signal_from_sensor['Sensor_nums'].append(sens_nums_array)

    return expected_signal_from_sensor


def transform_calc(accels, alphas, times, start_times, start_time_indices, expected_signal_from_sensor):
    SNR = []
    ...
    sensors_pos = expected_signal_from_sensor['cut_sensor_positions']

    sensors_nums = expected_signal_from_sensor['Sensor_nums']
    ...
    for i, start_time in enumerate(tqdm(start_times)):
        ...
        for k, expected_signal in enumerate(Signals_list):
            ...
            if n_steps_max > 0 and expected_signal.shape[0] != 0:
                ...
                for num, sensor_pos in enumerate(sensors_pos[k]):

                    sens_num = sensors_nums[k][num]

                    signal_from_sensor = accels[sens_num][
                                         start_index:start_index + expected_signal[num].shape[0]]

                    S_this_track += np.einsum(
                        'ij,ij->', expected_signal[num], signal_from_sensor
                    )
